...
export default {
  loadItems: async ({ commit, state }, { page, search }) => {
    ...
    let response = await integration.targets.getAll({ take: state.take, skip: state.skip, search: state.search });
  ...
  loadOneMoreItem: async ({ state }) => {
    ...
    let response = await integration.targets.getAll({ take: 1, skip: state.skip + state.take - 1, search: state.search });
    if (response.status !== 200) {
      throw new Error(`undefined error in backend (${response.status})`);
    }
