...
export default {
    loadItems: async ({ commit, state }, { page, search }) => {
        ...
        let response = await integration.courses.getAll({ take: state.take, skip: state.skip, search: state.search });
    ...
    loadItem: async ({ state }, { id }) => {
        ...
        let response = await integration.courses.get(id);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    loadOneMoreItem: async ({ state }) => {
        ...
        let response = await integration.courses.getAll({ take: 1, skip: state.skip + state.take - 1, search: state.search });
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    addItem: async ({ state }, { item }) => {
        ...
        let response = await integration.courses.post(item);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    updateItem: async ({ state }, { item }) => {
        ...
        let response = await integration.courses.put(item);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    deleteItem: async ({ state }, { id }) => {
        ...
        let response = await integration.courses.delete(id);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
