...
export default {
    loadItems: async ({ commit, state }, { page, search, parentId }) => {
        ...
        let response = await integration.questions.getAll({ scaleId: parentId, take: state.take, skip: state.skip, search: state.search });
    ...
    loadItem: async ({ state }, { id }) => {
        ...
        let response = await integration.questions.get(id);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    loadOneMoreItem: async ({ state }, { parentId }) => {
        ...
        let response = await integration.questions.getAll({ scaleId: parentId, take: 1, skip: state.skip + state.take - 1, search: state.search });
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    addItem: async ({ state }, { item }) => {
        ...
        let response = await integration.questions.post(item);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    updateItem: async ({ state }, { item }) => {
        ...
        let response = await integration.questions.put(item);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
    ...
    deleteItem: async ({ state }, { id }) => {
        ...
        let response = await integration.questions.delete(id);
        if (response.status !== 200) {
            throw new Error(`undefined error in backend (${response.status})`);
        }
